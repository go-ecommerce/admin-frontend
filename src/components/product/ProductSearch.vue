...
const props = defineProps<{
  modelValue?: VariantCardResponse[]
}>()

const emit = defineEmits<{
  (e: 'update:modelValue', value: VariantCardResponse[]): void
}>()

const selectedProducts = ref<VariantCardResponse[]>(props.modelValue ?? [])
const searchQuery = ref('')
const searchResults = ref<VariantCardResponse[]>([])
...
watchDebounced(
  ...
  async (query) => {
    ...
    try {
      loading.value = true
      const results = await ProductService.searchVariants(query)
...
const addProduct = (product: VariantCardResponse): void => {
  const updated = [...selectedProducts.value, product]
...
const thumbSrc = (product: VariantCardResponse) => imageSrc(product.image, 'thumb')
  ...
  <div class="relative">
    ...
    >
      ...
      <template v-else>
        ...
        >
          <img
            v-if="thumbSrc(product)"
            :src="thumbSrc(product)"
            :alt="product.image?.alt || product.name || ''"
            class="size-9 rounded object-cover shrink-0 border"
          />
          ...
          <Plus
            class="h-4 w-4 shrink-0 text-muted-foreground opacity-0 group-hover/item:opacity-100 transition-opacity"
          />
        </button>
      </template>
    </div>
